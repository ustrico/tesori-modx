...
(function($, undefined) {
	window.touchSlider = function(options) {
		...
		var ret = {
			...
			indexNow = $(options.indexNow, container);	
        ...
        moving: false,
				...
				to: function(toIndex, opt) {
					...
					toComplete = function() {
						...
						autoPlay();
            $('div.touchslider-text:visible').hide();
						$('div.touchslider-panel').appendTo($('div.touchslider-item:eq(' + slides.index(node) + ')'));
						$('div.touchslider-text:eq(' + slides.index(node) + ')').show();
	...
	$.fn.touchSlider = function(options) {
		...
		touchSlider(options);
		$('div.touchslider-text').hide();
		$('div.touchslider-text:first').show();
		return this;
	};
}(jQuery));
