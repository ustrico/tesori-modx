...
class LoginRegisterProcessor extends LoginProcessor {
    ...
    public function setUserFields() {
		...
		$extended['shirtname'] = $extended['firstname'];
		$fullname = $extended['firstname'];
		if (!empty($extended['lastname'])) {
			$extended['shirtname'] .= ' ' . mb_substr($extended['lastname'], 0, 1, 'UTF-8') . '.';
			$fullname .= ' ' . $extended['lastname'];
		}
		$this->profile->set('extended', $extended);
		$this->profile->set('fullname', $fullname);
